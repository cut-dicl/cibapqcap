...
pricenew=[];
 ...
 for n = 1:numvess % For every appliancesv...
...
iii      ;      
new_minpriceLocation=find(price2 == min(price2));
if length(new_minpriceLocation)>1
new_loc=1;
data.PBP(n)=sum(WharfLength(1:data.PBQ(n)-1))+new_loc; % adding new PBP due to overlaping
else
