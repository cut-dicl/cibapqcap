function [PBQ ABQ PBP]=PBP_PBQ_function_2CT(LoS, PBQ_name, AT, LoW)

% preferred berthing position and Converting PBQ (charecter value) to PBQ (number value)
for i=1:length(AT)

 if PBQ_name(i) =="Container/ Ro-Ro Quay" && length(LoW)==1% length 450
                PBQ(i) =1;
                ABQ(i) =1; % alternative berthing quay for 1st (container/ ro-ro quay)
                PBP(i)=randi([0, LoW(PBQ(i))-LoS(i)]); % preferred berthing position is generated based on preferred quay
            elseif PBQ_name(i) =="Container/ Ro-Ro Quay" && length(LoW)>1% length 450
                PBQ(i) =1;
            ...
            elseif PBQ_name(i) =="West Quay" && length(LoW)==5 % length 770
                PBQ(i) =4;
                ABQ(i) =5;
                PBP(i)=randi([sum(LoW(1:PBQ(i)-1))+1, sum(LoW(1:PBQ(i)))-LoS(i)]); % preferred berthing position is generated based on preferred quay

            elseif PBQ_name(i) =="West Quay" && length(LoW)==4 % length 770 % WHEN we are considering only 4 quays then NO ABQ no. 5
                PBQ(i) =4;
                ABQ(i) =4;
                PBP(i)=randi([sum(LoW(1:PBQ(i)-1))+1, sum(LoW(1:PBQ(i)))-LoS(i)]); % preferred berthing position is generated based on preferred quay
