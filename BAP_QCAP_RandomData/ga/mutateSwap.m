function [solution ] = mutateSwap(solution, data, LoW,cranes_numbers, tm, iteration)
...
if max( sum(LoW(1:PBQ(rand2)-1)), (prefferingBerth(rand2)-M)) > min(sum(LoW(1:PBQ(rand2)))-data.LoS(rand2),  prefferingBerth(rand2)+M)
    rand4=randi([(sum(LoW(1:PBQ(rand2)))-data.LoS(rand2))-M , sum(LoW(1:PBQ(rand2)))-data.LoS(rand2)]);
else
