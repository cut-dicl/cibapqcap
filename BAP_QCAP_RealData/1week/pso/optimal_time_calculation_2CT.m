function [Optimal_processing_time PBP possible_cranes cranes_numbers]=optimal_time_calculation_2CT(Load, max_cranes, HP,pTime,PBQ, PBP, LoS, LoW,PBQ_name,Total_containers)
...
for i=1:length(Load)
    if max_cranes(i)>0 && PBQ_name(i)=="Container Quay"
    ...
    elseif max_cranes(i)>0 && PBQ_name(i)=="Container/ Ro-Ro Quay"
        if PBP(i)+LoS(i)<200 
            possible_cranes(i)=20; % in fact this is 1 on this quay
            cranes_numbers(i)="1";
        elseif PBP(i)>300
            possible_cranes(i)=20; % 
            cranes_numbers(i)="2";
        else
            possible_cranes(i)=40; % 
            cranes_numbers(i)="12";
        end
