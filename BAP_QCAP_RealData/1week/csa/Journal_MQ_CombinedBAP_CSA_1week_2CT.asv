...
prompt = "ENTER THE NUMBER OF WEEKS (It maybe 1 OR 2 OR 4) = ";
user_input = input(prompt);
if user_input==1
    week=1; DataTable = readtable(AAAA,'Range','A1:M29'); 
elseif user_input==2
    week=2; DataTable = readtable(AAAA,'Range','A1:M69'); 
elseif user_input==4
    week=4; DataTable = readtable(AAAA,'Range','A1:M139'); 
else
    disp('PLEASE ENTER THE CORRECT NUMBER OF WEEKS (It maybe 1 OR 2 OR 4');
    return
end
...
[AT_30min, dep_30min, pTime]=dataModifiy_2CT(DataTable); % calling the function that calculates time values in 30-min time unit
