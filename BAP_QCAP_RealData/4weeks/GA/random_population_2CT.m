function [BT BP BQ NC pTime_new]=random_population_2CT(LoS, AT, departure, pTime, LoW, PBP, PBQ, nPop, Load, max_cranes, HP, cranes_numbers,Total_containers); % NC --> number of cranes
M=40; 
...
if xx(i)==0;    NC(k,i)=0; pTime_new(k,i)=pTime(i);
    ...
    aa=num2str(NC(k,i)); 
    for a=1:length(aa)
        if aa(a)=='1' || (PBQ(i)==1 && aa(a)=='2')
            HP_new1=20; % if first old crane is used.. productiivty will be 20
        else
            HP_new1=25; % if all other cranes is used.. productiivty will be 25
        end
        HP_new=HP_new+HP_new1;
    end
