...
function [bestNest_so_Far,min_ObjVal, nest, D, pa]=cuckoo_search4(nest, noOfGeneration, n, data, LoW, pTime_new,cranes_numbers,Total_containers) %this function ends on line 97
...
PBQ_changed=[];
%[cost]=qqq(Quay, LoW);
[fmin, bestNest, nest, fitness1, data,PBQ_changed]=get_best_nest4(nest,nest1,fitness, data, LoW,count,PBQ_changed, pTime_new);
...
%%%%%%%%%%%%%%%%%%%############### SECOND STEP generete new solutions
    % Generate new solutions (but keep the current best)
     new_nest=get_cuckoos4(nest,bestNest,data,LoW,PBQ_changed,pTime_new,cranes_numbers,count);   
     pTime_new=pTime_new_calculation4(data, n, new_nest,Total_containers); % this function calculates new processing time based on modified number of cranes


     [fnew1,best1,nest,fitness, data,PBQ_changed]=get_best_nest4(nest,new_nest,fitness, data, LoW,count,PBQ_changed, pTime_new);
    % Update the counter

      
      
    % Discovery and randomization
      new_nest=empty_nests(nest,best1,Lb,Ub,pa,data,cranes_numbers,count) ; 
      pTime_new=pTime_new_calculation4(data, n, new_nest,Total_containers); % this function calculates new processing time based on modified number of cranes    
    % Evaluate this set of solutions
     [fnew2,best2,nest,fitness, data,PBQ_changed]=get_best_nest4(nest,new_nest,fitness, data, LoW,count,PBQ_changed, pTime_new);
      fnew2;
...
function new_nest=empty_nests(nest,best1,Lb,Ub,pa,data,cranes_numbers,iterations)
...
if rand<pa
       ...
       for i=1:length(AT)
           ...
           if iterations<250
                BP_newNest(i)=randi([round((nest(j,length(AT)+i)+best1(length(AT)+i))/2), max(nest(j,length(AT)+i)+50, best1(length(AT)+i))+50]);
           else
                BP_newNest(i)=randi([best1(length(AT)+i)-10, best1(length(AT)+i)+10]) ; % modified line
           end
