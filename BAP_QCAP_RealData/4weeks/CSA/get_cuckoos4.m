...
function new_nest=get_cuckoos4(nest,best, data,LoW,PBQ_changed,pTime_new,cranes_numbers,iterations)
...
if j>2 && rand>0.5
    ...
    BQ=best(length(BT)*2+1:end-length(BP)); % storing before last 28 values in BP
    ...
    stepsize= (ones(size(BT)))/2;%0.01*step.*(s-best);
    ...
    %BP(i)
    if iterations<250
        new_BP(i)=randi([min(PBP(i),round(BP(i)+v(i)+step(i))), max(PBP(i),round(BP(i)+v(i)+step(i)))]) ; % this is working good 
    else
        %new_BP(i)=randi([min(BP(i),round(BP(i)+v(i)+step(i))), max(BP(i),round(BP(i)+v(i)+step(i)))]) ; % modified line
        new_BP(i)=randi([BP(i)-10, BP(i)+10]) ; % modified line
    end
