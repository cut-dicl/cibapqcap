...
function [PSO_BestPosition PSO_BestCost Quay]=PSO(data, LoW,Load,max_cranes,HP,cranes_numbers,Total_containers);
...
CostFunction=@(data,x,LoW,it,i,PBQ_changed,pTime_new) fitnessfunction(data,x,LoW,it,i,PBQ_changed,pTime_new);        % Cost Function

%% PSO Parameters
nVar=length(AT)*4;            % Number of Decision Variables
...
[BT BP BQ NC pTime_new]=random_population_2CT(LoS, data.AT, data.dep, pTime, LoW, PBP, PBQ, nPop, Load, max_cranes, HP, cranes_numbers,Total_containers); % random population generation % NC is number of cranes 
NewPop=[BT BP BQ NC];
...
for i=1:nPop
    ...
    % Evaluation
    particle(i).Cost=CostFunction(data,x,LoW,it,i,PBQ_changed,pTime_new(i,:));
...
for it=1:MaxIt
    ...
    for i=1:nPop
        ...
        bq=xx(length(AT)*2+1:length(AT)*3);
        nc=xx(length(AT)*3+1:end);
        %for gg=1:length(AT)
         %   bp(gg)=bp(gg)*randi([1,3]);
        %end

        xx=[bt bp bq nc];
        [pTime_new NC_new1]=pTime_new_calculation(data, nPop, xx,Total_containers,cranes_numbers); % this function calculates new processing time based on modified number of cranes
        nc=NC_new1; xx=[bt bp bq nc];

        [Cost pos PBQ_changed data] = CostFunction(data,xx,LoW, it,i,PBQ_changed,pTime_new);
